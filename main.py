    ...
    for post in posts:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                for i in entries:
                    content = re.sub(
                        " +", " ", html.unescape(i.html().replace('\n', ' ')))
                    exp = re.search(
                        'se confirma[a-z]+ (?P<new>([0-9]+|[a-z ]+)) (?:nuevo|caso)', content)
                    new = exp.group('new') if exp else 0
                    exp = re.search('acumulado de (?P<total>[0-9]+)', content)
                    total = exp.group('total') if exp else 0
                ...
                for i in entries:
                    content = strip_tags(
                        re.sub(" +", " ", html.unescape(i.html().replace('\n', ' '))))
                ...
                json.dump({'day': date, 'total': total, 'new': new, 'persons': persons}, open(
                    '{}.json'.format(str(date)), 'w'))
